...
PLOT_OPTIONS = {
    'lines': {'A': {'pen': {'color': 'w', 'width': 2.}},
              ...
              'C': {'pen': {'color': (255, 200, 0), 'width': 2.}},
              'D': {'pen': {'color': (255, 200, 0), 'width': 2.}}
             },
    'marks': {'A': {'pen': None, 'symbol': histogram_symbol,
              ...
              'C': {'pen': None, 'symbol': histogram_symbol,
                    'symbolPen': 'w', 'symbolSize': 2},    
              'D': {'pen': None, 'symbol': histogram_symbol,
                    'symbolPen': 'w', 'symbolSize': 2},        
             },
}
...
class UserInterface(QtWidgets.QMainWindow):
    ...
    def __init__(self, use_fake=False):
        ...
        self._pulseheights = {'A': [], 'B': [], 'C':[], 'D':[]}
        self._baselines = {'A': [], 'B': [], 'C':[], 'D':[]}
        ...
        if not self.is_run_time_completed():
            ...
            if self._write_output:
                ...
                writer.writerow(('time_A','pulse_height_A',
                                 ...
                                 'time_C','pulse_height_C',
                                 'time_D','pulse_height_D',))
    ...
    def set_upper_trigger_state(self, state):
        if not self._trigger_channel == 'A OR B OR C OR D':
            self._is_upper_threshold_enabled = state
            self.scope.stop()
    ...
    def set_channel(self):
        ...
        self.scope.set_channel('B', self._coupling, self._range,
                               self._polarity_sign * self._offset)
        self.scope.set_channel('C', self._coupling, self._range,
                               self._polarity_sign * self._offset)
        self.scope.set_channel('D', self._coupling, self._range,
                               self._polarity_sign * self._offset)                                             
    ...
    @QtCore.pyqtSlot()
    def fetch_data(self):
        t, [A,B,C,D] = self.scope.get_data()
        if A is not None:
            self.num_events += len(A)
            self.plot_data_signal.emit({'x': t, 'A': A, 'B': B, 'C' : C, 'D' : D })
        if self._is_running:
    ...
    def plot_data(self, data):
        ...
        if self._write_output and not self._output_file.closed:
            writer = csv.writer(self._output_file)
            for row in zip(times[0], pulseheights[0], times[1], pulseheights[1], times[2], pulseheights[2], times[3], pulseheights[3]):
                writer.writerow(row)

        if self._is_upper_threshold_enabled:
            if not self._trigger_channel == 'A OR B OR C OR D':
                channel_idx = ['A', 'B', 'C', 'D'].index(self._trigger_channel)
        ...
        for channel, tvalues, blvalues, phvalues \
            in zip(['A', 'B', 'C', 'D'], times, baselines, pulseheights):
            self._baselines[channel].extend(blvalues)
            self._pulseheights[channel].extend(phvalues)

        if len(A) > 0:
            self.update_event_plot(x, A[-1], B[-1], C[-1], D[-1], pulseheights[:, -1],
                                   baselines[:, -1])
    ...
    def update_event_plot(self, x, A, B,C, D, pulseheights, baselines):
        self.event_plot.clear()
        if self.ch_A_enabled_box.isChecked():
            ...
            self.event_plot.plot(x * 1e6, C, **self._plot_options['C'])
            self.event_plot.plot(x * 1e6, D, **self._plot_options['D'])
